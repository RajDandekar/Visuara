...
# ╔═╡ bde6f720-1329-11ec-0460-d367f2d2c914
begin
    using PlutoUI
	using Plots
...
# ╔═╡ 4978cb8b-54c2-4f99-8deb-7575bd8ab502
begin
...
ds2 = DiscreteDynamicalSystem(newton_map, [0.1, 0.2], [3.0], newton_map_J)
xg = yg = range(-1.5,1.5,length=400)
basins, attractors = basins_of_attraction((xg, yg), ds2; show_progress = false);
	
function scatter_attractors(attractors)
    for k ∈ keys(attractors)
        x, y = columns(attractors[k])
        PyPlot.scatter(x, y; color = "C$(k-1)", edgecolor = "white")
    end
end
...
# ╔═╡ 37385a7d-540e-42fc-a670-781b66f2c501
begin
...
# ╔═╡ 3e558fe5-5ae5-41d2-b88b-6a14e555028e
begin
	md""" # Interactive, visual notebooks"""
end

# ╔═╡ cfff5a5d-54a9-4bcb-bb30-3188f761872a
begin
	md"### Physical world

	Dynamical systems, chaos theory and much more!"
end

# ╔═╡ 49286ddf-f28f-444e-9fe2-eaeeda78b950
begin
	...
	function animation2()
		anim2 = @animate for i = 1:frames
			orbit_points = i:frames:(orbit_length*frames)
			
    Plots.scatter(a[orbit_points, 1], a[orbit_points, 2], markersize=1,              html_output_format=:png,leg=false, framestyle=:none, xlims=extrema(a[:,1]),  ylims=extrema(a[:,2]))
			
		end every 1
		gif(anim2, "anim2_fps15.gif", fps = 5)
	end
	
	animation2()
end

# ╔═╡ ed089bf2-b058-4bbd-a101-3e6a802a3755
@bind color_lorenz2 Select(["purple", "green","blue", "black","magenta"])

# ╔═╡ 45907c6b-15f0-45a4-ab1c-c8cb52f676cc
begin
...
anim3 = @animate for i=1:1500
    step!(attractor)
    push!(plt, attractor.x, attractor.y, attractor.z)
end every 10
gif(anim3, "anim3_fps15.gif", fps = 5)
end

# ╔═╡ 10cef652-0988-4f5c-ab19-9fdaa9f2cb23
begin
	n_timepoints = 500
	time_first_inf = 30
	n_first_inf = 20
	...
	Learning epidemic spread visually"
end

# ╔═╡ 5539029e-0c44-4cb0-a8bd-0231e97bbc72
begin
...
# ╔═╡ c493c377-a2cd-4db4-9ae1-c15858dc45ae
begin
	...
	Plots.plot(ex_inf + ex_susc, ribbon=(ex_susc, floor_plot), label="Susceptible")
	Plots.plot!(ex_inf, ribbon=(ex_inf, floor_plot), xlabel="Time",
		ylabel="Population", label="Infected", title="")
	Plots.plot!(ex_inf + ex_susc + ex_rec, ribbon=(ex_rec, floor_plot), label="Recovered")
	Plots.plot!(ceiling_plot, ribbon=(ex_dec, floor_plot), label="Deceased")
end

# ╔═╡ 1f430302-5f9d-485b-bc6c-c2ffd7819317
md"### Mathematical world

	Vector addition"

# ╔═╡ 09a73f2d-6b87-45ed-8c35-8900635786e9
corgis = load(download("https://user-images.githubusercontent.com/6933510/108605549-fb28e180-73b4-11eb-8520-7e29db0cc965.png"))

# ╔═╡ 132b158c-9727-41d7-b2af-57ff105b8390
upsidedown_corgis = corgis[ end:-1:1 , :]

# ╔═╡ 56250c3d-7690-4d42-9f58-a71af258ea5f
    @bind α Slider(0:.01:1 , show_value=true, default = 1.0)

# ╔═╡ afd0fde4-a687-457c-a7e0-545aa4b90baf
...
git-tree-sha1 = "32f4b3df43c2d2125a4dc5f72d5c5ef8a066fd2b"
...
git-tree-sha1 = "63c9ae7dad3e9840b41188d5229833314f4457f7"
...
git-tree-sha1 = "ffa83c8c07f28c9a02ec5a342ad7280870220819"
...
git-tree-sha1 = "693f6396eebe934e9685d1d016854f07187ec8a5"
...
git-tree-sha1 = "1b7ece34a907d0493902772dfb3bf5708557c094"
...
git-tree-sha1 = "8e75757ace35afef78d9676d32d8413c95c0eb1f"
